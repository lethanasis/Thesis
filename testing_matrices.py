...
def construct_matrix(ions: IonHandler, ne, Te, Z, fre=None, V_plasma=1, V_vessel=1):
    ...
    for ion in ions:
        ...
        for j in range(ion.Z+1):
            ...
            if j == 0 :
                A[N-1, off+j] -= Di /(dist**2) * (Te-0.025) * scipy.constants.e
...
# Add fast-electron impact ionization
            ...
            #Replacing the equation for fully ionized Ne with the total Ne density condition
        #A[1, N-1] = 0 
...
def construct_F(ions: IonHandler, ne, Te, Z, fre=None):
    ...
    for ion in ions:
        ...
        if ion.name not in ['H', 'D']:
            F[N-3] = ion.n
    #set element for neutral pressure condition
    F[1]=pn
    
    for ion in ions:
        F[1] -= Te*ion.solution[0]*scipy.constants.e
        if ion.name not in ['H', 'D']:
            for j in range(ion.Z+1):
                F[N-3] -= ion.solution[j]
